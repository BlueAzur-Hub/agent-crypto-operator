(() => {
  ...
  const PROFILE = Object.freeze({
    id: "yohan-operator-admin-406054",
    build: "40.6.54",
    baseBuild: "40.6.53",
  ...
  function hideProjectsSurface() {
    qsa("button, a, [role='button'], article, span").forEach((node) => {
      ...
      if (/^projets?\b/i.test(value) || /portfolio/i.test(value)) {
        ...
        target.dataset.yohanHidden406054 = "1";
      }
    });
  }
    ...
    if (footer) {
      qsa("span", footer).forEach((item) => {
        if (includes(item, "PROJETS") || includes(item, "PORTFOLIO")) {
          ...
          item.dataset.yohanHidden406054 = "1";
        }
      });
    }
  }
  ...
  function ensureOperatorDefault() {
    ...
    document.documentElement.dataset.yohanProfile406054 = "operator";
    document.documentElement.dataset.yohanOperatorBuild = "40.6.54";
    if (document.body) {
      document.body.dataset.yohanProfile406054 = "operator";
      document.body.dataset.yohanOperatorBuild = "40.6.54";
    }
  }
  ...
  globalThis.ErithYohanOperatorAdmin406054 = Object.freeze({
    profile: PROFILE,
    apply,
    hideProjectsSurface,
    adaptAccessPortal
